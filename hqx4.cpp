...
/*inline uint32_t hqx4::rgb_to_yuv(uint32_t c)
{
    // Mask against MASK_RGB to discard the alpha channel
    return hqx::RGBtoYUV[MASK_RGB & c];
}
inline int hqx4::Diff(uint32_t c1, uint32_t c2)
{
    return hqx4::yuv_diff(rgb_to_yuv(c1), rgb_to_yuv(c2));
}
// Test if there is difference in color
inline int hqx4::yuv_diff(uint32_t yuv1, uint32_t yuv2) {
    return (( abs((yuv1 & Ymask) - (yuv2 & Ymask)) > trY ) ||
            ( abs((yuv1 & Umask) - (yuv2 & Umask)) > trU ) ||
            ( abs((yuv1 & Vmask) - (yuv2 & Vmask)) > trV ) );
}
*/
void hqx4::hq4x_32_rb( uint32_t * sp, uint32_t srb, uint32_t * dp, uint32_t drb, int Xres, int Yres )
{
    int  i, j, k;
...
void hqx4::hq4x_32( uint32_t * sp, uint32_t * dp, int Xres, int Yres )
{
